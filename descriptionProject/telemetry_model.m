...
%% Rev A power rails and protection model
power_cfg = struct( ...
    'vin_v', 9.0, ...
    'vin_min_v', 7.0, ...
    'vin_max_v', 12.0, ...
    ...
    'v5v5_tol', 0.05, ...
    'v3v3_tol', 0.03, ...
    'input_current_limit_a', 1.0, ...
    'usb_current_limit_a', 0.1, ...
    'board_power_w', 4.5, ...
    'board_power_limit_w', 6.6, ...
    'converter_efficiency', 0.85);

power_cfg.input_current_a = power_cfg.board_power_w / ...
    (power_cfg.vin_v * power_cfg.converter_efficiency);
power_cfg.power_fault = power_cfg.vin_v < power_cfg.vin_min_v || ...
    power_cfg.vin_v > power_cfg.vin_max_v || ...
    power_cfg.input_current_a > power_cfg.input_current_limit_a || ...
    power_cfg.board_power_w > power_cfg.board_power_limit_w;
power_status = 'ok';
if power_cfg.power_fault
    power_status = 'fault';
end
...
json_status = sprintf([...
    ...
    '  },\n',...
    '  "power": {\n',...
    '    "voltage_v": %.2f, "input_voltage_v": %.2f, "status": "%s",\n',...
    '    "v3v3_v": %.2f, "v1v2_v": %.2f, "input_current_a": %.3f\n',...
    '  },\n',...
    '  "fan": {\n',...
